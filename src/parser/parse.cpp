...
ConstructorDecl* Parser::createAutogeneratedConstructor(TypeDecl* typeDecl) const {
    ...
    for (auto& field : typeDecl->fields) {
        ...
        auto* right = field.defaultValue ? field.defaultValue : new VarExpr(field.getName().str(), field.getLocation());
        body.push_back(new ExprStmt(new BinaryExpr(Token::Assignment, left, right, field.getLocation())));

        if (!field.defaultValue) {
            params.push_back(ParamDecl(field.type, field.getName().str(), false, field.getLocation()));
        }
    }

    auto* autogeneratedInit = new ConstructorDecl(*typeDecl, std::move(params), typeDecl->accessLevel, typeDecl->getLocation());
